@extends("nav")
@section('title', 'Bill Status | Senior Life Care Trusts')
@section("wrapper")
<?php
    use App\Models\User;
                ...
                <div class="card mt-2 mb-xl-0">
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-lg-12 pt-0 pb-2">
                            ...
                            <input class="manual-check-ctrl" {{ $claim->recurring_bill==1 ? 'checked' : ' ' }} value="{{ 1 }}" name="recurring_bill" type="checkbox" @if ($claim->claim_status != 'Pending') disabled @endif id="recurring_bill">
                        </div>
                        <div class="col-lg-12 pt-2 pb-2 recurring">
                            ...
                            <select class="form-control @if ($claim->claim_status == 'Pending') select-2 @endif" name="recurring_day" @if ($claim->claim_status != 'Pending') disabled @endif>
                                ...
                                @for ($i = 1; $i <= 28; $i++) <option value="{{ $i }}" @if ($claim->recurring_day == $i) selected @endif>{{ $i }}{{ in_array($i % 100, [11, 12, 13]) ? 'th' : ($i % 10 == 1 ? 'st' : ($i % 10 == 2 ? 'nd' : ($i % 10 == 3 ? 'rd' : 'th'))) }} of every Month</option>
                                    @endfor
                            </select>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        <h4>Bill Info</h4>
                        <div class="col-lg-3 pe-0" style=";margin-left:auto">
                            @if ($role != "User" && $claim->claim_status == 'Pending')
                            <select style="background-image: url('data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22180%22 height=%22180%22 viewBox=%220 0 24 24%22%3E%3Cpath fill=%22white%22 d=%22M7 10l5 5 5-5z%22/%3E%3C/svg%3E');
    background-repeat: no-repeat;
    background-position: right 10px center;
    background-size: 20px 20px; /* Set the icon size to 80x80 */
    padding-right: 50px;" id="bill_status" class="form-control form-select text-white @if ($claim->claim_status == 'Approved' || $claim->claim_status == 'Partially approved') bg-success @endif @if ($claim->claim_status == 'Pending') bg-primary @endif @if ($claim->claim_status == 'Refused') bg-danger @endif" name="claim_status">
                    ...
                    <div class="row refusal_div {{$claim->refusal_reason!=null ? '' : 'd-none'}} ">
                        ...
                        <div class="col-sm-9 text-secondary ">
                            <textarea class="form-control" id="exampleFormControlTextarea1" name="refusal_reason" rows="3" @if ($claim->claim_status == 'Approved' || $claim->claim_status=='Refused') readonly @endif  @if ($role == "User") readonly @endif>{{ $claim->refusal_reason }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="row partial_div {{$claim->partial_amount!=null ? '' : 'd-none'}} ">
                        ...
                        <div class="col-sm-4 text-secondary">
                            <input class="form-control" name="partial_amount" type="number" value="{{$claim->partial_amount}}" step="any" @if ($claim->claim_status != 'Pending') disabled @endif id="" >
                        </div>
                    </div>
                    ...
                    <div class="row partial_div {{$claim->partial_amount!=null ? '' : 'd-none'}}">
                        ...
                        <div class="col-sm-9 text-secondary ">
                            <textarea class="form-control" id="exampleFormControlTextarea1" name="reason" @if ($claim->claim_status != 'Pending') disabled @endif     rows="3"
                    ...
                    <div class="row due-date @if (!$claim->expense_date) d-none @endif">
                        ...
                        <div class="col-sm-4 text-secondary">
                            <input type="date" name="expense_date" value="{{date('Y-m-d',strtotime($claim->expense_date))}}" class="form-control" @if ($role=="User" ) readonly @endif @if ($claim->claim_status != 'Pending') disabled @endif
                            />
                ...
                <div class="row">
                    ...
                    <div class="col-sm-9 text-secondary">
                        <textarea class="form-control" id="exampleFormControlTextarea1" name="claim_description" @if ($claim->claim_status != 'Pending') disabled @endif     rows="3"
